...
class Model(torch.nn.Module):
    ...
    def set_time_dict(self, time_dict):
        """Set time dictionary for temporal ordering"""
    ...
    def forward(self, data):
        ...
        # Sắp xếp seq_x theo thời gian
        sorted_seq_x = []
        for b_i in range(len(seq_x)):
            if self.time_dict is not None and b_i in self.time_dict:
                # Get timestamps for current batch
                times = []
                for i in range(len(seq_x[b_i])):
                    if i in self.time_dict[b_i]:
                        times.append(self.time_dict[b_i][i])
                    else:
                        # Use a default timestamp for missing entries
                        times.append(datetime.datetime.min)

                # Sort indices by timestamp
                sorted_indices = sorted(range(len(times)), key=lambda k: times[k])
                sorted_seq_x.append(seq_x[b_i][sorted_indices])
            else:
                # If no time info available, keep original order
                sorted_seq_x.append(seq_x[b_i])

        # Tiếp tục xử lý với seq_x đã sắp xếp
        if self.pre_padding:
            sorted_seq_x = tuple(map(lambda s: s.flip(0), sorted_seq_x))
            seq_x = pad_sequence(sorted_seq_x, batch_first=True)
            seq_x = seq_x.flip(1)
        else:
            seq_x = pad_sequence(sorted_seq_x, batch_first=True)
...
dataset = FNNDataset(root=r'C:\20241\DATN\DATN\data', feature=args.feature, empty=False, name=args.dataset,
